...
docker service create --name logspout \
  ...
  gliderlabs/logspout syslog+tcp://{logstash_ip}:514
```

**使用**

- logspout可以忽略特定的容器，不收集它们的日志，只需要在运行容器时添加一个环境变量`LOGSPOUT=ignore`。
...
2. 添加日志解析配置 **/data1/logstash/service.conf**
...
3. 修改集群上所有主机的配置参数(ElasticSearch需求)
